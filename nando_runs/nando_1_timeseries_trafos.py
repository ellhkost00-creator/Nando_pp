    ...
    while True:
        try:
            ...
            if str(user_input).upper() in options:
                ...
                current_directory = r"C:\Users\anton\Desktop\nando_pp\excels"
...
class DSSDriver:
    ...
    def Voltge_profile_plot(self):
        ...
        # PLOT
        legend = []
        fig1, ax1 = plt.subplots(dpi=100, figsize=(6, 3))
    ...
    def Voltge_timeseries_plot(self):
        ...
        fig, axes = plt.subplots(dpi=150, figsize=(8, 3))
        ...
        # Mean per timestep
        # =========================
        mean_per_timestep = self.Vdata.mean(axis=1)
        ...
        legend_elements = [
            ...
            Line2D([0], [0], color='black', lw=2.5, label='Mean voltage'),
    ...
    def Tx_utilisation_plot(self):
        ...
        if (self.data["lvtx"]["Conn_Type"].str.len() == 3).all():
            ...
            fig, axes = plt.subplots(dpi=150, figsize=(8, 3))  # Adjust figsize as needed
...
def run_daily(temp_dss_ckt2):
        ...
        txs_name = temp_dss_ckt2.dss_circuit.Transformers.AllNames
...
# Ensure numeric
S_lv_txs = S_lv_txs.apply(pd.to_numeric, errors="coerce")
S_mv_tx = S_mv_tx.apply(pd.to_numeric, errors="coerce")

# Add timestamps
S_lv_txs_out = S_lv_txs.copy()
S_lv_txs_out.index = time_index
S_lv_txs_out.index.name = "timestamp"

S_mv_tx_out = S_mv_tx.copy()
S_mv_tx_out.index = time_index
S_mv_tx_out.index.name = "timestamp"

# =========================
# 2) Get rated kVA from your stored data (same logic as Tx_utilisation_plot)
# =========================
lvtx_df = temp_dss_ckt2.data["lvtx"].copy().reset_index(drop=True)

# find a name column (if exists) to match by name; else fallback to same-order
name_col = None
for cand in ["DSSNAME", "dssname", "Name", "name", "Element_Name", "element_name"]:
    if cand in lvtx_df.columns:
        name_col = cand
        break

tx_caps = {}  # map: trafo_name(lower) -> rated_kVA

if name_col is not None:
    # Match by name
    lvtx_df["_name"] = lvtx_df[name_col].map(norm)
    lvtx_df["_kva"] = pd.to_numeric(lvtx_df["kvas_primary"], errors="coerce")

    rated_map = (
        lvtx_df.dropna(subset=["_name", "_kva"])
              .drop_duplicates(subset=["_name"])
              .set_index("_name")["_kva"]
              .to_dict()
    )

    for tx in S_lv_txs_out.columns:
        tx_caps[norm(tx)] = float(rated_map.get(norm(tx), np.nan))
else:
    # Fallback: assume same order as your plot loop
    # (i-th column in S_lv_txs corresponds to i-th row in lvtx)
    for i, tx in enumerate(S_lv_txs_out.columns):
        if i < len(lvtx_df):
            tx_caps[norm(tx)] = float(pd.to_numeric(lvtx_df.loc[i, "kvas_primary"], errors="coerce"))
        else:
            tx_caps[norm(tx)] = np.nan

# =========================
# 3) Compute loading percent per trafo
# =========================
loading_percent = S_lv_txs_out.copy()

missing = []
for tx in loading_percent.columns:
    cap = tx_caps.get(norm(tx), np.nan)
    if cap is None or np.isnan(cap) or cap <= 0:
        loading_percent[tx] = np.nan
        missing.append(tx)
    else:
        loading_percent[tx] = 100.0 * loading_percent[tx] / cap

# =========================
# 4) Summary table
# =========================
summary = pd.DataFrame(index=S_lv_txs_out.columns)
summary.index.name = "trafo"
summary["rated_kVA"] = [tx_caps.get(norm(tx), np.nan) for tx in summary.index]
summary["max_loading_%"] = loading_percent.max(axis=0, skipna=True)
summary["mean_loading_%"] = loading_percent.mean(axis=0, skipna=True)
summary["hours_over_100%"] = (loading_percent > 100).sum(axis=0) * 0.5  # 30min -> 0.5h

missing_df = pd.DataFrame({"missing_trafos_in_ratings": missing})

# =========================
# 5) Write Excel with sheets
# =========================
with pd.ExcelWriter(OUT_XLSX, engine="openpyxl") as writer:
    S_mv_tx_out.to_excel(writer, sheet_name="S_mv_tx_kVA")
    S_lv_txs_out.to_excel(writer, sheet_name="S_lv_txs_kVA")
    loading_percent.to_excel(writer, sheet_name="loading_percent")
    summary.to_excel(writer, sheet_name="summary")
    missing_df.to_excel(writer, sheet_name="missing_in_ratings", index=False)

print("Saved:", OUT_XLSX)
if missing:
    print(f"WARNING: {len(missing)} trafos δεν βρέθηκαν/δεν είχαν kVA. Δες sheet 'missing_in_ratings'.")