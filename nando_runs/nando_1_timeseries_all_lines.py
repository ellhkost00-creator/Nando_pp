...
def run_daily(temp_dss_ckt2):
        ...
        # Line currents
        all_lines = temp_dss_ckt2.dss_circuit.Lines.AllNames
        temp_dss_ckt2.Idata = pd.DataFrame(index=range(48), columns=all_lines)        
        ...
        for it in tqdm(range(48), desc="Solving time-series power flow"):
            ...
            for line_name in temp_dss_ckt2.dss_circuit.Lines.AllNames:
                ...
                if temp_phases == "1":
                    ...
                    av_current = (
                        float(temp_currents[0]) +
                        float(temp_currents[2]) +
                        float(temp_currents[4])
                    ) / 3
...
def export_all_line_loading_csv(
    ...
    """
    Exports loading percent (%) per timestep for ALL OpenDSS lines (MV + LV) to CSV.

    Assumptions:
    - Idata columns are OpenDSS line names as used in run_daily()
    - Ampacity is read directly from each OpenDSS line property: normamps
    """

    # Build ampacity map directly from OpenDSS
    ampacity_dict = {}
    ...
    for line_name in temp_dss_ckt2.dss_circuit.Lines.AllNames:
        ...
        try:
            ampacity_dict[line_name] = float(
                temp_dss_ckt2.dss_circuit.ActiveElement.Properties("normamps").Val
            )
        except:
    ...
    # Warn if any line has missing ampacity
...
loading_pct = export_all_line_loading_csv(
    ...
    Idata=Idata,
    out_csv=r"C:\Users\anton\Desktop\nando_pp\excels\all_lines_loading_percent.csv",
    stepsize_minutes=30
)

print("Saved:", loading_pct.shape)