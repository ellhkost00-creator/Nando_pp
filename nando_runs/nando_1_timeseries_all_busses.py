...
def run_daily(temp_dss_ckt2):
        ...
        # Get all actual DSS nodes
        all_node_names = [str(x).strip() for x in temp_dss_ckt2.dss_circuit.AllNodeNames]

        # Keep only phase nodes (.1 .2 .3), not neutrals / .0
        all_voltage_nodes = []
        for node in all_node_names:
            parts = node.split(".")
            if len(parts) >= 2 and parts[1] in {"1", "2", "3"}:
                all_voltage_nodes.append(node)

        # remove duplicates while keeping order
        all_voltage_nodes = list(dict.fromkeys(all_voltage_nodes))
        ...
        # Output variables
        # Voltage info
        temp_dss_ckt2.Vdata = pd.DataFrame(index=range(48), columns=all_voltage_nodes)
        # Active Power info
        ...
        for it in tqdm(range(48), desc="Solving time-series power flow"):
            ...
            # for all loads
            temp_dss_ckt2.Vdata.loc[it, :] = temp_all_V_values.loc[all_voltage_nodes, "Val"].values
