...
class DSSDriver:
    ...
    def lv_load_mod(self, selected_day=0):
        ...
        house_data = np.load(r"C:\Users\anton\Desktop\nando_pp\excels\Res_load_data_30min_res.npy")
        com_data = np.load(r"C:\Users\anton\Desktop\nando_pp\excels\Com_load_data_30min_res.npy")
        time_res = 30
        if selected_day==0:
            np.random.seed(100)
...
def export_mv_line_loading_excel(
    temp_dss_ckt2,
    Idata: pd.DataFrame,
    out_xlsx: str,
    ...
    """
    Exports MV line loading (%) per timestep to Excel.

    Assumptions:
    - Idata columns are OpenDSS line names (lower/any case) as used in run_daily()
    - temp_dss_ckt2.gis_data["MV_lines"] has columns: "DSSNAME", "Ampacity"
    - "Ampacity" is in Amps
    """

    mv_lines = temp_dss_ckt2.gis_data["MV_lines"].copy()

    # Build mapping: line_name(lower) -> ampacity
    mv_lines["DSSNAME_lower"] = mv_lines["DSSNAME"].astype(str).str.lower()
    amp_map = mv_lines.set_index("DSSNAME_lower")["Ampacity"]

    # Align ampacity series to Idata columns
    idata_cols = pd.Index([str(c).lower() for c in Idata.columns])
    ampacity_aligned = amp_map.reindex(idata_cols)

    # Optional: warn for missing ampacity
    missing = ampacity_aligned[ampacity_aligned.isna()].index.tolist()
    if missing:
        print(f"[WARN] Missing Ampacity for {len(missing)} lines (will become NaN loading):")
        print(missing[:20], "..." if len(missing) > 20 else "")

    # Compute loading %
    # (broadcasting: each column divided by its ampacity)
    loading_pct = Idata.copy()
    loading_pct.columns = idata_cols  # normalize
    loading_pct = loading_pct.divide(ampacity_aligned, axis=1) * 100.0

    # Create a time index (optional, nicer than 0..47)
    time_index = pd.date_range(
        "2021-01-01 00:00",
        periods=len(loading_pct),
        freq=f"{stepsize_minutes}min"
    )
    loading_pct.index = time_index
    current_A = Idata.copy()
    current_A.columns = idata_cols
    current_A.index = time_index

    # Daily max table
    daily_max = pd.DataFrame({
        "ampacity_A": ampacity_aligned.values,
        "max_current_A": current_A.max(axis=0).values,
        "max_loading_pct": loading_pct.max(axis=0).values,
    }, index=idata_cols)
    daily_max.index.name = "line"

    # Ampacity table
    ampacity_tbl = ampacity_aligned.rename("ampacity_A").to_frame()
    ampacity_tbl.index.name = "line"

    # Write Excel
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        loading_pct.to_excel(writer, sheet_name="loading_pct")
        current_A.to_excel(writer, sheet_name="current_A")
        ampacity_tbl.to_excel(writer, sheet_name="ampacity_A")
        daily_max.to_excel(writer, sheet_name="daily_max")

    return loading_pct, daily_max


# ---- Usage ----
V_loads, P_loads, Q_loads, S_mv_tx, S_lv_txs, Idata = run_daily(temp_dss_ckt2)

loading_pct, daily_max = export_mv_line_loading_excel(
    temp_dss_ckt2=temp_dss_ckt2,
    Idata=Idata,
    out_xlsx=r"C:\Users\anton\Desktop\nando_pp\excels\mv_line_loading.xlsx",
    stepsize_minutes=30
)
print("Saved:", loading_pct.shape, daily_max.shape)